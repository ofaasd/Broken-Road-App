...
<script>
	...
	const edit = () => { 
	    ...
	    .pushPage('edit.html')
	    .then(function(){
	    	const url = "https://genius.telkomsolo.com/api/pelanggan/"+myid;
			fetch(url,{
			  	method : "GET",
			  	headers : {
			  		"Authorization" : localStorage.token,
...
//		    		alert(obj['status']);
	...
	function search(){
		...
		const by = $('#by').val();
		const url = "https://genius.telkomsolo.com/api/pelanggan/cari?search="+keyword+"&by="+by;
		fetch(url,{
		  	method : "GET",
		  	headers : {
		  		"Authorization" : localStorage.token,
